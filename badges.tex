...
\newcommand{\BadgeWidth}{6.0cm}
\newcommand{\BadgeHeight}{9.0cm}


% Distance between badges.  You probably do not need to change this.
\newcommand{\BadgeSepX}{0.5cm}
\newcommand{\BadgeSepY}{0.6cm}
...
\pgfmathsetlengthmacro{\BadgeMarginTop}{(\paperheight - 3 * \BadgeHeight - 3 * \BadgeSepY) / 3}
\pgfmathsetlengthmacro{\BadgeMarginLeft}{(\paperwidth - 3 * \BadgeWidth - \BadgeSepX) / 3}
\pgfmathsetlengthmacro{\LogoHeight}{\BadgeHeight * 0.15}
\pgfmathsetlengthmacro{\LogoWidth}{\BadgeWidth * 0.8}
...
\newcommand{\attendance}[5]{
  \pgfmathsetlengthmacro{\xshift}{1 * \BadgeWidth / 3}
  \begin{scope}[xshift=0*\xshift]
    \draw [fill=#2!20,draw=none] (#4, -\Fudge) rectangle (\BadgeWidth / 1 + #5, 0.5);
    \node [anchor=base] at (2\BadgeWidth / 8,0.12) {\textbf{\large{#3}}};
  \end{scope}
}
...
\begin{document}
  ...
  \csvreader{participants.csv}{%
    1=\affiliation,3=\firstname,4=\lastname,
    5=\eventA,6=\eventB,7=\eventC,8=\eventD}%
  {
